...
class Matrix {
  ...
  getColumn(columnNumber) {
    const result = [];
    this.forEach((element, [row, column]) => columnNumber === column && result.push(element));
    return result;
  }
  ...
  multipleBy(multiplier) {
    ...
    } else if (multiplier instanceof Matrix && Matrix.areMultiCompatible(this, multiplier)) {
  ...
  #multipleByMatrix(inputMatrix) {
    ...
    for (let column = 0; column < inputMatrix.columnsNumber; column += 1) {
      for (let row = 0; row < this.rowsNumber; row += 1) {
        ...
        result[row][column] = AugumentedArray.multiplyArrays(this.value[row], inputColumnt);
      }
    }
    return result;
  }
  ...
  static isValidMatrix(source) {
    if (AugumentedArray.isArrayOfArrays(source)) {
      ...
      return source.every(
        (element) => element.length === rowLength && AugumentedArray.isArrayOfNumbers(element)
      );
    }
  }
}
